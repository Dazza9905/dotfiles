...
export SSH_AUTH_SOCK=/home/dazza/.bitwarden-ssh-agent.sock
...
nvim_switch_configuration() {
  local config target nvim_path
  nvim_path="${XDG_CONFIG_HOME:-$HOME/.config}/nvim"

  config="$(
    fd -d 1 -t d . "${XDG_CONFIG_HOME:-$HOME/.config}/nvims" |
      xargs -n1 basename |
  ...
  target="${XDG_CONFIG_HOME:-$HOME/.config}/nvims/$config"
